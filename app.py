...
def ordenar_pontos(pts):
    """Ordena os 4 cantos: [superior-esquerdo, superior-direito, inferior-direito, inferior-esquerdo]"""
...
def alinhar_perspectiva(image):
    """Localiza os 4 marcadores quadrados pretos nos cantos da folha e retifica a imagem."""
    ...
    for c in contours:
        ...
        # Filtra por área compatível com os quadrados de canto
        if 200 < area < (h_img * w_img * 0.05):
            peri = cv2.arcLength(c, True)
            approx = cv2.approxPolyDP(c, 0.04 * peri, True)
            if len(approx) == 4:  # Quadriláteros
                M = cv2.moments(c)
                if M["m00"] != 0:
                    cX = int(M["m10"] / M["m00"])
                    cY = int(M["m01"] / M["m00"])
    ...
    if len(centros) >= 4:
        ...
        # Pega os 4 marcadores mais extremos
        rect = ordenar_pontos(centros[:4])
        
        # Dimensão fixa de destino para padronização
    ...
    # Caso não encontre os 4 cantos, altera apenas o tamanho base
...
if uploaded_file is not None:
    ...
    if not alinhado:
        st.warning("⚠️ Marcadores de canto não foram detectados com clareza. Tente tirar a foto com mais iluminação e enquadrando os 4 cantos da folha.")
